...
- capability: UserLists
  location: server/routes/lists.js, server/models/List.js, src/pages/MyLists.js, src/pages/MyFavorites.js
  status: healthy
  scope: all
  owner: User Data
  public_api: api.get('/lists'), api.post('/lists'), api.post('/lists/:id/items'), api.delete('/lists/:id')
  description: Kullanıcıların özel ve sistem listeleri (favoriler vb.) üzerinde CRUD işlemleri.
